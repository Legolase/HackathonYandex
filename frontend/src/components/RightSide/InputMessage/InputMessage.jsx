...
const InputMessage = ({dialog}) => {
    ...
    return (<>
            <form ref={formRef} className={cl.messageInput} onSubmit={(event) => submitForm(event)} method={'post'}>

                {showPicker &&
                    <div className={cl.emojiPicker}>
                        <EmojiPicker
                            onEmojiClick={(event) => setInputValue(prevInput => prevInput + event.emoji)}
                            searchDisabled={true}
                            emojiStyle="native"/>
                    </div>
                }
                ...
                <button type={"button"} className={cl.button} onClick={() => setShowPicker(val => !val)}
                        style={{backgroundColor: 'transparent', border: 'none', width: '45px', height: '35px'}}>
                    <svg viewBox="0 0 49 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                ...
                <button type={"submit"} className={cl.button}>
                    <svg className={cl.svg} viewBox="0 0 24 24" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
